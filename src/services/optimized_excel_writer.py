...
class OptimizedExcelWriter(BaseService):
    ...
    def _init_styles(self):
        ...
        self.border_styles = {
            'all_thin': Border(left=self.thin_side, right=self.thin_side, 
                              top=self.thin_side, bottom=self.thin_side),
            'all_thick': Border(left=self.thick_side, right=self.thick_side,
                               top=self.thick_side, bottom=self.thick_side),
            'top_left_thick': Border(left=self.thick_side, top=self.thick_side,
                                    right=self.thin_side, bottom=self.thin_side),
            'top_right_thick': Border(right=self.thick_side, top=self.thick_side,
                                     left=self.thin_side, bottom=self.thin_side),
            'bottom_left_thick': Border(left=self.thick_side, bottom=self.thick_side,
                                       right=self.thin_side, top=self.thin_side),
            'bottom_right_thick': Border(right=self.thick_side, bottom=self.thick_side,
                                        left=self.thin_side, top=self.thin_side),
        }
    ...
    def write_unassigned_vehicles_optimized(
        ...
        # Write headers
        headers = [
            "Van ID", "Vehicle Type", "Operational Status", "Last Known Location",
            "Days Since Last Assignment", "VIN", "GeoTab Code", "Branded or Rental",
            "Notes", "Unassigned Date", "Unassigned Time"
        ]
