...
class BorderFormattingService(BaseService):
    ...
    def create_daily_section(
        ...
        self._apply_section_border(sheet, start_row, start_col, end_row, end_col, thick_side)

        # Add section header with date
        header_row = start_row
        self._create_section_header(sheet, header_row, start_col, end_col, section_date, title)
    ...
    def _apply_section_border(
        ...
        cell.border = Border(
            top=border_side, left=border_side, right=cell.border.right, bottom=cell.border.bottom
        )

        # Top-right
        cell = sheet.cell(row=start_row, column=end_col)
        cell.border = Border(
            top=border_side, right=border_side, left=cell.border.left, bottom=cell.border.bottom
        )

        # Bottom-left
        cell = sheet.cell(row=end_row, column=start_col)
        cell.border = Border(
            bottom=border_side, left=border_side, top=cell.border.top, right=cell.border.right
        )

        # Bottom-right
        cell = sheet.cell(row=end_row, column=end_col)
        cell.border = Border(
            bottom=border_side, right=border_side, top=cell.border.top, left=cell.border.left
        )
    ...
    def _create_section_header(
        ...
        header_cell.border = Border(
            top=medium_side, left=medium_side, right=medium_side, bottom=medium_side
        )
    ...
    def highlight_cells(
        self, sheet: Any, cells: list[tuple[int, int]], color: str = "FFFF00"
    ) -> None:
