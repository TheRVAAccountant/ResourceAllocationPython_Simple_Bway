...
class DailyDetailsWriter(BaseService):
    ...
    def validate(self) -> bool:
        ...
        if len(self.DAILY_DETAILS_COLUMNS) != 24:
            logger.error(
                f"Invalid DAILY_DETAILS_COLUMNS: expected 24, got {len(self.DAILY_DETAILS_COLUMNS)}"
            )
            return False
    ...
    def append_to_existing_file(
        ...
        try:
            ...
            new_rows = self._prepare_allocation_rows(
                allocation_result, allocation_date, vehicle_log_dict, existing_ids
            )
            ...
            if new_rows:
                ...
                self.thick_border_service.apply_thick_borders_after_append(
                    daily_details, start_row, len(new_rows)
                )
            else:
            ...
            if not skip_results_sheet:
                ...
                if results_sheet_name not in workbook.sheetnames:
                    self._create_results_sheet(
                        workbook, results_sheet_name, allocation_result, allocation_date
                    )

            # Create Unassigned sheet if needed
            if allocation_result.unallocated_vehicles:
                unassigned_sheet_name = self._create_dated_sheet_name(
                    allocation_date, "Available & Unassigned"
                )
    ...
    def generate_unique_identifier(
        self, allocation_date: date, route: str, device: str, van_id: str
    ) -> str:
        ...
        if detailed_results:
            ...
            for result in detailed_results:
                ...
                unique_id = self.generate_unique_identifier(
                    allocation_date, route_code, vehicle_id, vehicle_id  # Device ID  # Van ID
                )
                ...
                if vehicle_log_dict and vehicle_id in vehicle_log_dict:
                    ...
                    row[7] = self._normalize_brand_display(
                        brand_rental_value
                    )  # Type (Branded or Rental)
            ...
            for driver_id, vehicles in allocation_result.allocations.items():
                for _index, vehicle_id in enumerate(vehicles):
                    ...
                    unique_id = self.generate_unique_identifier(
                        allocation_date, route_code, vehicle_id, vehicle_id
                    )
                    ...
                    if vehicle_log_dict and vehicle_id in vehicle_log_dict:
                        ...
                        row[7] = self._normalize_brand_display(
                            vehicle_info.get("brand_or_rental", "")
                        )  # Type
        ...
        if detailed_results:
            ...
            for driver_id, vehicles in allocation_result.allocations.items():
                for vehicle_id in vehicles:
                    unique_id = self.generate_unique_identifier(
                        allocation_date, f"R{row_num-1:03d}", vehicle_id, vehicle_id
                    )
