...
class DailyDetailsThickBorderService:
    ...
    def apply_thick_borders_after_append(
        ...
        """
        logger.info(f"Applying thick borders to {num_new_rows} new rows starting at row {new_data_start_row}")
        ...
        if current_date and section_start_row <= new_data_start_row + num_new_rows - 1:
            self._apply_thick_border_to_section(
                worksheet,
                section_start_row,
                new_data_start_row + num_new_rows - 1,
                current_date
            )
    
    def _extend_previous_section(
        self,
        worksheet: Worksheet,
