...
class AllocationHistoryService(BaseService):
    ...
    def initialize(self) -> None:
        ...
        logger.info(f"AllocationHistoryService initialized (max_entries={self.max_entries}, "
    ...
    def save_allocation(
        ...
        try:
            ...
            if metadata:
                ...
                unallocated_count = metadata.get('unallocated_count', metadata.get('total_unassigned'))
            ...
            if metrics_obj is not None:
                total_routes = total_routes if total_routes is not None else getattr(metrics_obj, 'total_vehicles', None)
                allocated_count = allocated_count if allocated_count is not None else getattr(metrics_obj, 'allocated_vehicles', None)
                unallocated_count = unallocated_count if unallocated_count is not None else getattr(metrics_obj, 'unallocated_vehicles', None)
                allocation_rate = allocation_rate if allocation_rate is not None else (float(getattr(metrics_obj, 'allocation_rate', 0)) * 100)
                total_drivers = total_drivers if total_drivers is not None else getattr(metrics_obj, 'total_drivers', None)
                active_drivers = active_drivers if active_drivers is not None else getattr(metrics_obj, 'active_drivers', None)
                processing_time = processing_time if processing_time is not None else getattr(metrics_obj, 'processing_time', None)
    ...
    def get_history(
        ...
        try:
            ...
            for entry in history[:limit]:
                dup_count, dup_details = self._parse_duplicate_conflicts(entry.get("duplicate_conflicts"))
                formatted_entry = {
                    ...
                    "metrics": {
                        ...
                        "success_rate": entry.get("allocation_rate", 0.0) / 100.0,  # Convert to 0-1 range
    ...
    def get_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            duplicate_count = sum(self._duplicate_conflict_count(e.get('duplicate_conflicts')) for e in recent)
            
            return {
                "total_allocations": total_allocations,
                "success_rate": round(success_count / total_allocations, 3) if total_allocations > 0 else 0.0,
