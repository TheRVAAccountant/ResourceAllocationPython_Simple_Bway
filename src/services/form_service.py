...
class FormService(BaseService):
    ...
    def _load_default_forms(self):
        ...
        # Vehicle entry form
        vehicle_form = ExcelForm("vehicle_entry", "Vehicle Entry Form", "Enter vehicle information")

        vehicle_form.add_field(
            FormField(
                "vehicle_number",
                "Vehicle Number",
                "text",
        ...
        # Driver entry form
        driver_form = ExcelForm("driver_entry", "Driver Entry Form", "Enter driver information")

        driver_form.add_field(FormField("employee_id", "Employee ID", "text", required=True))

        driver_form.add_field(FormField("name", "Driver Name", "text", required=True))

        driver_form.add_field(
            FormField(
                "location",
                "Location",
                "dropdown",
        ...
        try:
            ...
            for field in form.fields:
                ...
                if field.field_type == "dropdown" and field.options:
                    dv = DataValidation(
                        type="list", formula1=f'"{",".join(field.options)}"', showDropDown=True
                    )
                ...
                # Style input cell
                ws[input_cell].fill = PatternFill(
            ...
            ws[f"A{current_row + 1}"].fill = PatternFill(
