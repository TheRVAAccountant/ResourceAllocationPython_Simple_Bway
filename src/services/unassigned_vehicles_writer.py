...
class UnassignedVehiclesWriter(BaseService):
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if not isinstance(self.enable_alternating_rows, bool):
                logger.error(f"Invalid enable_alternating_rows value: {self.enable_alternating_rows}")
                return False
            ...
            if self.historical_data_path is not None:
                if not isinstance(self.historical_data_path, str):
                    logger.error(f"Invalid historical_data_path type: {type(self.historical_data_path)}")
                    return False
    ...
    def _write_unassigned_data(
        ...
        for _, vehicle in unassigned_vehicles.iterrows():
            ...
            days_since_assignment = self.calculate_days_since_assignment(
                van_id,
                historical_assignments
            ) if historical_assignments is not None else 0
    ...
    def calculate_days_since_assignment(
        ...
        try:
            ...
            vehicle_assignments = historical_data[
                historical_data["Van ID"] == vehicle_id
            ]
            ...
            logger.warning(
                f"Error calculating days since assignment for {vehicle_id}: {e}"
            )
            return 0
    
    def create_unassigned_summary(
        self,
        unassigned_vehicles: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Unassigned summary: {total_unassigned} total, "
            f"{operational_count} operational"
        )
