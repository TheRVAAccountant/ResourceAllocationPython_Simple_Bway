...
from src.models.allocation import AllocationRequest, AllocationResult, Vehicle, Driver
...
class AllocationEngine(BaseService):
    ...
    def allocate(self, request: AllocationRequest) -> AllocationResult:
        ...
        """
        logger.info(f"Starting allocation for {len(request.vehicles)} vehicles "
        ...
        logger.info(f"Allocation completed: {len(allocations)} assignments, "
    ...
    def _update_metrics(
        ...
        self.metrics = AllocationMetrics(
            ...
            allocation_rate=Decimal(str(allocated_vehicles / total_vehicles)) if total_vehicles > 0 else Decimal("0"),
            average_vehicles_per_driver=Decimal(str(allocated_vehicles / len(allocations))) if allocations else Decimal("0"),
