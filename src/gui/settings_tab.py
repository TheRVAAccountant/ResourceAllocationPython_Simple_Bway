...
class SettingsTab:
    ...
    def __init__(self, parent, allocation_engine, excel_service, on_settings_saved: Optional[Callable[[dict], None]] = None):
    ...
    def create_setting_row(
        ...
        **kwargs
        ...
        if widget_type == "entry":
            ...
            # Add up/down buttons
            up_btn = ctk.CTkButton(frame, text="▲", width=30, height=15,
                                  command=lambda: self.spin_value(widget, 1, kwargs))
            up_btn.grid(row=0, column=1, padx=(2, 0))
            
            down_btn = ctk.CTkButton(frame, text="▼", width=30, height=15,
                                    command=lambda: self.spin_value(widget, -1, kwargs))
            ...
            widget = ctk.CTkSlider(
                parent,
                from_=kwargs.get("min_value", 0),
                to=kwargs.get("max_value", 100)
            )
            widget.grid(row=row, column=1, sticky="ew", padx=10, pady=8)
    ...
    def create_action_buttons(self):
        ...
        self.save_button = ctk.CTkButton(
            ...
            text="💾 Save Settings",
            width=130,
            command=self.save_settings_to_file
        )
        ...
        for attr in dir(self):
            ...
            try:
                if isinstance(current, bool):
                    value = bool(raw) if not isinstance(raw, str) else raw.lower() in ("1", "true", "yes", "on")
