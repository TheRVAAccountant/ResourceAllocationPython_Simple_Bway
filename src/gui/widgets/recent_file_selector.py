...
class RecentFileSelector(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        self.entry = ctk.CTkEntry(
            self, textvariable=self.textvariable, placeholder_text=self.placeholder_text, height=40
        )
        ...
        self.browse_button = ctk.CTkButton(
            self, text="Browse", width=100, height=40, command=self._browse_file
        )
    ...
    def _update_recent_button_state(self):
        ...
        if recent_files:
            ...
            self.recent_button.configure(state="normal", text_color=("gray10", "gray90"))
        else:
            # Disable with visual feedback
            self.recent_button.configure(state="disabled", text_color=("gray60", "gray40"))
    ...
    def _show_recent_files(self):
        ...
        def destroy_popup(event):
            ...
            if (
                self.recent_popup
                and self.recent_popup.winfo_exists()
                and not str(widget).startswith(str(self.recent_popup))
            ):
                self.recent_popup.destroy()
                self.recent_popup = None
                # Unbind this event
        ...
        self.after(
            100,
            lambda: setattr(
                self, "_popup_bind_id", self.winfo_toplevel().bind("<Button-1>", destroy_popup, "+")
            ),
        )
