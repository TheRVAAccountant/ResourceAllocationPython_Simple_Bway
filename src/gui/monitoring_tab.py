...
class MonitoringTab(ctk.CTkFrame):
                    ...
                    if key == 'overall_status':
                        ...
                        if isinstance(check_data, dict):
                            if 'usage_percent' in check_data:
                                status_text = f"{status.title()} ({check_data['usage_percent']:.1f}%)"
                            else:
            ...
            if 'metrics' in perf_data:
                for name, data in perf_data['metrics'].items():
                    ...
                    self.perf_tree.insert('', 'end', values=(
                        display_name, avg_val, min_val, max_val, latest_val, unit
                    ))
    ...
    def _refresh_alerts_data(self):
        ...
        try:
            ...
            # Get active alerts
            active_alerts = [
                alert for alert in self.monitoring_service.active_alerts.values()
    ...
    def _clear_resolved_alerts(self):
        ...
        try:
            ...
            resolved_keys = [
                key for key, alert in self.monitoring_service.active_alerts.items()
                if alert.resolved
            ]
