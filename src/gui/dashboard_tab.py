...
class MetricCard(ctk.CTkFrame):
    ...
    def __init__(self, parent, title: str, value: str = "0", subtitle: str = "", color: str = "blue"):
...
class DashboardTab:
    ...
    def __init__(self, parent, allocation_engine, dashboard_data_service=None, daily_summary_path_getter: Optional[Callable[[], Optional[str]]] = None):
    ...
    def update_metrics(self):
        ...
        try:
            ...
            if self.dashboard_data_service:
                ...
                try:
                    total_vehicles_from_file = self.dashboard_data_service.total_operational_vehicles(ds_path)
            ...
            if self.allocation_engine:
                ...
                if total_drivers_from_file is not None:
                    self.total_drivers_card.update_value(
                        str(total_drivers_from_file),
                        f"{metrics.active_drivers} active"
                    )
                else:
                    self.total_drivers_card.update_value(
                        str(metrics.total_drivers),
                        f"{metrics.active_drivers} active"
                    )
                
                self.allocated_card.update_value(
                    str(metrics.allocated_vehicles),
                    "Vehicles assigned"
                )
                
                rate_percent = float(metrics.allocation_rate) * 100
                self.allocation_rate_card.update_value(
                    f"{rate_percent:.1f}%",
                    "Success rate"
                )
                
                self.unallocated_card.update_value(
                    str(metrics.unallocated_vehicles),
                    "Pending assignment"
                )
