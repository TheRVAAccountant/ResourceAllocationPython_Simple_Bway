...
def fix_fixture_references(file_path):
    ...
    replacements = [
        ...
        (
            r"def test_[^(]*\(self, validator,",
            r"def test_\g<0>".replace("validator,", "duplicate_validator,"),
        ),
        (r"validator\.", "duplicate_validator."),
        (r"sample_allocations", "sample_allocation_results"),
        # Unassigned writer tests
        (
            r"def test_[^(]*\(self, writer,",
            r"def test_\g<0>".replace("writer,", "unassigned_writer,"),
        ),
    ...
    if "test_duplicate_validator.py" in str(file_path):
        ...
        content = content.replace(
            "result = validator.validate_allocations(sample_allocations)",
        ...
        content = content.replace(
            "validator = DuplicateVehicleValidator", "validator = DuplicateVehicleValidator"
        )
        content = content.replace(
            "validator.validate_allocations", "validator.validate_allocations"
