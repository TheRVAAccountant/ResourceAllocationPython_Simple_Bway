...
class TestDateFormattingConsistency:
    ...
    def test_consistent_formatting_within_groups(self, daily_details_sheet):
        ...
        for date_val, rows in date_groups.items():
            ...
            first_fill = first_cell.fill.start_color.rgb if first_cell.fill and first_cell.fill.start_color else None
            ...
            for row in rows:
                ...
                cell_fill = cell.fill.start_color.rgb if cell.fill and cell.fill.start_color else None
                
                assert cell_bold == first_bold, \
                    f"Inconsistent bold formatting in group {date_val}: row {row} differs from row {rows[0]}"
                assert cell_fill == first_fill, \
    ...
    def test_gray_fill_applied_to_all_dates(self, daily_details_sheet):
        ...
        for row in range(2, daily_details_sheet.max_row + 1):
            ...
            if date_cell.value:
                has_gray_fill = (
                    date_cell.fill and 
                    date_cell.fill.start_color and 
                    date_cell.fill.start_color.rgb in ["FFE6E6E6", "00E6E6E6"]
                )
