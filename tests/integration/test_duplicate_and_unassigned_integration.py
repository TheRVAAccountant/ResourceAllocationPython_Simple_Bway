...
class TestDuplicateAndUnassignedIntegration:
    ...
    def test_peak_season_allocation_scenario(self, test_excel_files):
        ...
        peak_routes = pd.DataFrame([
            {"Route Code": f"CX{i}", "Service Type": "Standard Parcel - Large Van", 
             "DSP": "BWAY", "Wave": "8:00 AM", "Staging Location": f"STG.G.{i%5+1}"}
        ...
        driver_assignments = pd.DataFrame([
            {"Route Code": f"CX{i}", "Driver Name": f"Driver_{i}"}
            for i in range(1, 51)
        ])
    ...
    def test_vehicle_breakdown_scenario(self, test_excel_files):
        ...
        mixed_vehicles = pd.DataFrame([
            {"Van ID": f"BW{i}", "Type": "Large", 
             "Opnal? Y/N": "Y" if i % 3 != 0 else "N"}  # 1/3 are broken down
            for i in range(1, 31)
        ])
        
        routes_data = pd.DataFrame([
            {"Route Code": f"CX{i}", "Service Type": "Standard Parcel - Large Van",
             "DSP": "BWAY", "Wave": "8:00 AM", "Staging Location": f"STG.G.{i%3+1}"}
    ...
    def test_mixed_service_types_scenario(self, test_excel_files):
        ...
        mixed_routes = pd.DataFrame([
            {"Route Code": "CX1", "Service Type": "Standard Parcel - Large Van", 
             "DSP": "BWAY", "Wave": "8:00 AM", "Staging Location": "STG.G.1"},
            {"Route Code": "CX2", "Service Type": "Standard Parcel - Extra Large Van - US",
             "DSP": "BWAY", "Wave": "8:00 AM", "Staging Location": "STG.G.2"},
            {"Route Code": "CX3", "Service Type": "Standard Parcel Step Van - US",
             "DSP": "BWAY", "Wave": "9:00 AM", "Staging Location": "STG.G.3"},
            {"Route Code": "CX4", "Service Type": "Standard Parcel - Large Van",
             "DSP": "BWAY", "Wave": "9:00 AM", "Staging Location": "STG.G.4"}
        ])
        ...
        for result in allocation_results:
            ...
            if "Extra Large" in service_type:
                assert van_type == "Extra Large", f"Extra Large service should use Extra Large van, got {van_type}"
            elif "Step Van" in service_type:
                assert van_type == "Step Van", f"Step Van service should use Step Van, got {van_type}"
