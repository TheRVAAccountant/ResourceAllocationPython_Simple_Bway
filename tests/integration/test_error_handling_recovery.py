...
class TestErrorHandlingAndRecovery:
    ...
    def corrupted_excel_files(self, temp_dir):
        ...
        wrong_data = pd.DataFrame({
            "Wrong Col 1": ["data1", "data2"],
            "Wrong Col 2": ["data3", "data4"]
        })
    ...
    def test_missing_required_sheets(self, corrupted_excel_files):
        ...
        with pytest.raises(Exception) as exc_info:
            allocator.load_day_of_ops(str(corrupted_excel_files["missing_sheets"]), sheet_name="Solution")
    ...
    def test_unassigned_writer_with_corrupt_data(self, temp_dir):
        ...
        corrupted_vehicles = pd.DataFrame([
            ...
            {"Van ID": "BW3", "Type": "Step Van", "Opnal? Y/N": None}, # None operational status
        ])
    ...
    def test_data_validation_recovery(self):
        ...
        non_strict_validator = DuplicateVehicleValidator(config={"strict_duplicate_validation": False})
    ...
    def test_circular_reference_handling(self, temp_dir):
        ...
        try:
            ...
            # If it fails, should be a reasonable error
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in ["formula", "reference", "circular", "excel"])
    ...
    def test_concurrent_file_access_handling(self, test_excel_files):
        ...
        if errors:
            for error in errors:
                ...
                assert any(keyword in error.lower() for keyword in ["lock", "access", "permission", "sharing"])
        else:
