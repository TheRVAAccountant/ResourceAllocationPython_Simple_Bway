...
class ExcelStructureAnalyzer:
        ...
        for col in range(1, worksheet.max_column + 1):
            ...
            if value is not None:
                ...
                header_formatting[f'col_{col}'] = {
                    ...
                    'fill': {
                        'color': cell.fill.start_color.rgb if cell.fill and hasattr(cell.fill, 'start_color') else None
                    },
                    'alignment': {
    ...
    def _analyze_sample_data(self, worksheet, sheet_info: Dict, max_rows: int = 10) -> None:
        ...
        for row in range(2, min(max_rows + 2, worksheet.max_row + 1)):
            ...
            if non_empty_values:
                print(f"  Row {row}: {' | '.join(non_empty_values[:5])}{'...' if len(non_empty_values) > 5 else ''}")
    ...
    def _analyze_data_patterns(self, worksheet, sheet_info: Dict) -> None:
        ...
        for row in range(1, min(50, worksheet.max_row + 1)):
            ...
            if (cell_a.value and 
                cell_a.font and cell_a.font.bold and
                isinstance(cell_a.value, str)):
                ...
                if (cell_a.fill and hasattr(cell_a.fill, 'start_color') and 
                    cell_a.fill.start_color.rgb not in ['00000000', 'FFFFFFFF']):
                    is_section_header = True
                ...
                if is_section_header:
                    section_headers.append({
                        ...
                        'formatting': {
                            ...
                            'fill_color': cell_a.fill.start_color.rgb if cell_a.fill and hasattr(cell_a.fill, 'start_color') else None
                        }
                    })
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("="*80)
        report.append(f"EXCEL STRUCTURE ANALYSIS REPORT")
        ...
        for sheet_name, sheet_data in self.analysis_results['sheets_analysis'].items():
            ...
            if sheet_data['merged_cells']:
                ...
                for merge in sheet_data['merged_cells']:
                    report.append(f"  {merge['range']} ({merge['size']['rows']}×{merge['size']['cols']})")
    ...
    def save_analysis(self, output_file: Optional[str] = None) -> None:
...
def main():
    ...
    try:
        ...
        print(f"{'='*80}")
        print(f"Use the generated JSON file and this report to understand the")
        print(f"structure that should be replicated in your output files.")
